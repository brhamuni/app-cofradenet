...
import { User, ChevronDown, LogOut, ShieldCheck, LayoutDashboard, Newspaper, Calendar, Images } from 'lucide-react';
...
export default function UserDropdown({ onLogout, isAdmin }: { onLogout: () => void; isAdmin: boolean }) {
  ...
  return (
    <div className="relative shrink-0" ref={ref}>
      ...
      >
        <div className="h-9 w-9 rounded-full bg-cofrade-main text-white flex items-center justify-center font-black shadow-inner shrink-0">
          {isAdmin ? <ShieldCheck size={16} /> : <User size={18} />}
        </div>
        <ChevronDown size={14} className={`text-gray-400 transition-transform duration-300 shrink-0 ${isDropdownOpen ? 'rotate-180' : ''}`} />
      </button>

...
export class MarchasController {
    ...
    @ApiOperation({
        summary: 'Buscar marchas en el catálogo local y sugerencias externas',
    })
    @ApiResponse({
        status: 200,
        description: 'Resultados de la búsqueda inteligente de marchas',
    })
    @Get('search')
    ...
    @ApiResponse({
        status: 201,
        description: 'Marcha añadida al repertorio correctamente',
    })
    @ApiResponse({ status: 401, description: 'No autenticado' })
    @ApiResponse({
        status: 403,
        description: 'Solo las bandas pueden gestionar su repertorio',
    })
    @Post('repertorio')
    @Roles(RolUsuario.BANDA)
    @UseGuards(JwtAuthGuard, RolesGuard)
    async addRepertorio(@Body() dto: CreateMarchaDto, @Req() req: Request) {
    ...
    @ApiResponse({
        status: 201,
        description: 'Marcha añadida a favoritas correctamente',
    })
    @ApiResponse({ status: 401, description: 'No autenticado' })
    @ApiResponse({
        status: 403,
        description: 'Solo los cofrades pueden gestionar sus favoritas',
    })
    @Post('favoritas')
    @Roles(RolUsuario.COFRADE)
    @UseGuards(JwtAuthGuard, RolesGuard)
    async addFavorita(@Body() dto: CreateMarchaDto, @Req() req: Request) {
    ...
    @ApiResponse({
        status: 200,
        description: 'Marcha actualizada correctamente',
    })
    @ApiResponse({ status: 404, description: 'Marcha no encontrada' })
    @Patch(':id')
    ...
    @ApiOperation({
        summary: 'Eliminar una marcha del repertorio de una banda',
    })
    @ApiBearerAuth('access-token')
    @ApiResponse({
        status: 200,
        description: 'Marcha eliminada del repertorio correctamente',
    })
    @ApiResponse({ status: 401, description: 'No autenticado' })
    @ApiResponse({
        status: 403,
    ...
    @Roles(RolUsuario.BANDA)
    @UseGuards(JwtAuthGuard, RolesGuard)
    removeRepertorio(@Param('id') id: string, @Req() req: Request) {
        return this.marchasService.remove(+req.user!.id, +id, 'repertorio');
    }

    @ApiOperation({
        summary: 'Eliminar una marcha de las favoritas del usuario',
    })
    @ApiBearerAuth('access-token')
    @ApiResponse({
        status: 200,
        description: 'Marcha eliminada de favoritas correctamente',
    })
    @ApiResponse({ status: 401, description: 'No autenticado' })
    @ApiResponse({
        status: 403,
    ...
    @Roles(RolUsuario.COFRADE)
    @UseGuards(JwtAuthGuard, RolesGuard)
    removeFavorita(@Param('id') id: string, @Req() req: Request) {
    ...
    @ApiResponse({
        status: 200,
        description: 'Lista de marchas del repertorio de la banda',
    })
